...
import TikTokConfigPanel from './TikTokConfigPanel';

const API = '/api/tiktok';

const TikTokSidebar = ({ activeTab, setActiveTab, accounts, selectedAccountId, setSelectedAccountId, onBack }) => {
  const menuItems = [
  ...
  return (
    <div className="sidebar">
      <div className="sidebar-brand">
        ...
        <h2 style={{ background: 'linear-gradient(to right, #f953c6, #b91d73)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          TikTok AI
        </h2>
      </div>
      ...
      {accounts.length > 0 && (
        <div className="account-selector-container">
          <label className="text-xs opacity-50 ml-1">TIKTOK ACCOUNT</label>
          <select
          ...
          >
            {accounts.map(acc => (
              <option key={acc.id} value={acc.id}>{acc.name || acc.tiktok_open_id}</option>
            ))}
          </select>
        </div>
      )}
      ...
      <nav className="sidebar-nav">
        {menuItems.map(item => (
          <button
            ...
            style={activeTab === item.id ? { background: 'linear-gradient(135deg, #f953c6, #b91d73)', color: '#fff' } : {}}
...
const TikTokApp = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [accounts, setAccounts] = useState([]);
  const [selectedAccountId, setSelectedAccountId] = useState(1);
  const [status, setStatus] = useState({ schedules: [], tiktokToken: false, lastPost: null, automation_enabled: 'true' });
  ...
  const fetchAccounts = async () => {
    try {
      ...
      setFetchError('TikTok API connection failed: ' + e.message);
    }
  };
  ...
  const handlePostNow = async (customPrompt = null) => {
    setLoading(true);
    setMessage('🎠 Generating carousel...');
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage(`✅ Carousel posted! (${data.slideCount} slides)`);
        fetchData();
      } else {
  ...
  return (
    <>
      ...
      <div className="dashboard-container" style={{ background: 'radial-gradient(circle at 50% 0%, rgba(249,83,198,0.05), transparent)' }}>
        ...
        {activeTab === 'dashboard'
          ? <TikTokDashboard {...commonProps} handlePostNow={handlePostNow} history={history} />
          : <TikTokConfigPanel {...commonProps} />
        }
      </div>
    </>
  );
