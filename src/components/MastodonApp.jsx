...
import MastodonConfigPanel from './MastodonConfigPanel';

const API = '/api/mastodon';

const MastodonSidebar = ({ activeTab, setActiveTab, accounts, selectedAccountId, setSelectedAccountId, onBack }) => {
  const menuItems = [
    ...
    <div className="sidebar">
      ...
      {accounts.length > 0 && (
        <div className="account-selector-container">
          <label className="text-xs opacity-50 ml-1">MASTODON BLOG</label>
          <select
...
const MastodonApp = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [accounts, setAccounts] = useState([]);
  const [selectedAccountId, setSelectedAccountId] = useState(1);
  const [status, setStatus] = useState({ schedules: [], mastodonToken: false, lastPost: null, automation_enabled: 'true' });
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data === 'MASTODON_AUTH_SUCCESS') {
        setMessage('✅ Mastodon blog connected successfully!');
        fetchAccounts();
  ...
  const fetchAccounts = async () => {
    try {
      ...
      // Actually we need to add /api/mastodon/accounts in backend
      ...
      setFetchError('Mastodon API connection failed: ' + e.message);
    }
  };
  ...
  const handlePostNow = async (customPrompt = null) => {
    setLoading(true);
    setMessage('📝 Generating Mastodon post...');
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage(`✅ Post published on Mastodon!`);
        fetchData();
      } else {
  ...
  return (
    <>
      ...
      <div className="dashboard-container" style={{ background: 'radial-gradient(circle at 50% 0%, rgba(0,25,53,0.05), transparent)' }}>
        ...
        {activeTab === 'dashboard'
          ? <MastodonDashboard {...commonProps} handlePostNow={handlePostNow} history={history} />
          : <MastodonConfigPanel {...commonProps} />
        }
      </div>
    </>
  );
