...
const TumblrDashboard = ({ status, fetchData, loading, accountId, accounts, handlePostNow, history }) => {
    ...
    return (
      <div className="dashboard-content flex-center">
        <div className="glass-card" style={{ textAlign: 'center' }}>
          <h2>No Tumblr Blog Connected</h2>
          <p>Please go to Settings and connect your Tumblr blog via OAuth.</p>
        </div>
      </div>
    );
  }
      ...
      <header className="content-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p className="subtitle">Automate your Tumblr photo sets with AI.</p>
          </div>
          <button 
      ...
      <div className="main-grid">
        <div className="left-column">
          <section className="glass-card">
            ...
            <p className="section-desc">Trigger a post immediately to your Tumblr blog.</p>
                ...
                history.map(item => (
                  <div key={item.id} className="history-item animate-fade-in" style={{ borderLeftColor: item.status === 'success' ? '#10b981' : '#ef4444' }}>
                    ...
                    <p style={{ fontSize: '13px', margin: '0' }}>
                      {item.status === 'success' ? (
                        <>Post ID: <a href={`https://tumblr.com`} target="_blank" rel="noreferrer" style={{color: '#60a5fa'}}>{item.post_id}</a></>
                      ) : (
                        item.error_message
                      )}
                    </p>
...
export default TumblrDashboard;
