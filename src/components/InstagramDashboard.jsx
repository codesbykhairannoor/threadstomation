...
const InstagramDashboard = ({ status, handlePostNow, history, loading, statusLoading, accountId }) => {
  return (
    <div className="dashboard-content animate-fade-in">
      <header className="content-header">
        <h1>Instagram Automation Bot 📸</h1>
        <div className="status-header">
          <span className={`badge ${statusLoading ? 'badge-neutral' : status.instagramToken ? 'badge-success' : 'badge-error'}`}>
            {statusLoading ? '⏳ Checking...' : status.instagramToken ? '🔗 API Linked' : '🔴 API Offline'}
          </span>
          <button
      ...
      <div className="main-grid">
        <div className="left-column">
          <section className="glass-card mt-2">
            <h3>Quick Actions</h3>
            <p className="section-desc">Manually trigger an AI Instagram post right now.</p>
            <div className="flex-col gap-1">
              <button
                className="btn btn-glow w-full"
                onClick={() => handlePostNow()}
                disabled={loading || !status.instagramToken}
              >
                📸 Generate &amp; Post Now
              </button>
            </div>
            {!status.instagramToken && (
              <div className="tiktok-auth-hint">
                <p>⚠️ No Instagram account linked.</p>
          ...
          <section className="glass-card history-card">
            ...
            <div className="history-list">
              {history.length > 0 ? history.map(item => (
                <div key={item.id} className="history-item">
                  <div className="history-meta">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {item.slide_count && (
                        <span className="badge badge-neutral">Carousel ({item.slide_count} slides)</span>
                      )}
                ...
                <div className="empty-state">No Instagram posts yet.</div>
              )}
            </div>
          </section>
        </div>
