import axios from 'axios';

const TOKEN = '';

async function checkIG() {
  try {
    const res = await axios.get(`https://graph.facebook.com/v19.0/me?fields=id,name,instagram_accounts{id,username,connected_facebook_page{id,name}}&access_token=${TOKEN}`);
    console.log(JSON.stringify(res.data, null, 2));
  } catch (e) {
    console.error(e.response?.data || e.message);
  }
}

checkIG();
