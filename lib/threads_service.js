      ...
      if (token.length > 0 && token[0].access_token) {
        ...
        for (let i = 0; i < contents.length; i++) {
          ...
          try {
            ...
            await sql`
              INSERT INTO post_history (content, media_url, status, platform, threads_id, account_id) 
              VALUES (${currentContent}, ${currentImage}, 'success', 'threads', ${result.id}, ${accountId})
            `;
          } catch (err) {
            finalStatus = 'failed';
