...
export async function getInstagramAccessToken(accountId) {
  const rows = await sql`
    SELECT access_token, facebook_access_token, expires_at 
    FROM instagram_accounts 
    WHERE id = ${accountId}
  `;

  if (!rows.length || (!rows[0].access_token && !rows[0].facebook_access_token)) {
  ...
  const { access_token, facebook_access_token, expires_at } = rows[0];
  const active_token = access_token || facebook_access_token;
 ...
 */
export async function postInstagramReel(videoUrl, caption, accountId) {
  ...
  console.log(`[Instagram-Post] Creating Reels video container for account ${accountId}...`);
  const containerRes = await axios.post(`${base}/${instagramBusinessId}/media`, null, {
    params: {
      media_type: 'REELS',
      video_url: videoUrl,
      caption: caption,
      share_to_feed: 'true', // CRITICAL: Pushes Reel to profile feed and follower feeds, enabling seed audience impressions
      thumb_offset: 1000,   // Grab frame at 1s for thumbnail instead of blank/cut 0s
      access_token: accessToken,
    },
  });

  const containerId = containerRes.data.id;
  if (!containerId) throw new Error('Failed to retrieve Instagram Reels container ID');

  // Step 2: Wait for video processing
  await awaitContainerProcessing(accessToken, containerId, 20); // allow more attempts for video processing

  // Step 3: Publish container
  console.log(`[Instagram-Post] Publishing Reels container ${containerId}...`);
...
export async function postInstagramCarousel(imageUrls, caption, accountId) {
  ...
  const itemPromises = imageUrls.map(async (url, i) => {
    try {
      ...
      if (err.response) {
        console.error(`[Instagram-Post] Error creating item ${i}:`, JSON.stringify(err.response.data, null, 2));
      }
      throw err;
    }
  });
...
/**
 * Universal poster: handles either single image, video reel, or carousel.
 */
export async function postToInstagram(imageUrls, caption, accountId) {
  try {
    const url = Array.isArray(imageUrls) ? imageUrls[0] : imageUrls;
    if (!url) throw new Error('No media URL provided for Instagram post');

    if (url.includes('.mp4')) {
      return await postInstagramReel(url, caption, accountId);
...
export async function postFacebookVideo(videoUrl, caption, fbPageId, accessToken) {
  const base = 'https://graph.facebook.com/v19.0';
  console.log(`[Facebook-Post] Publishing video to page ${fbPageId}...`);
  const res = await axios.post(`${base}/${fbPageId}/videos`, null, {
    params: {
      file_url: videoUrl,
      description: caption,
      access_token: accessToken,
...
export async function postToFacebook(imageUrls, caption, accountId) {
  try {
    ...
    if (url.includes('.mp4')) {
      return await postFacebookVideo(url, caption, facebook_page_id, facebook_access_token);
    } else if (Array.isArray(imageUrls) && imageUrls.length > 1) {
...
/**
 * Fetches recent comments on an Instagram media object
 */
export async function fetchInstagramComments(mediaId, accountId) {
  try {
    const token = await getInstagramAccessToken(accountId);
    ...
    console.warn(`[Instagram-Comments] Failed to fetch comments for media ${mediaId}:`, e.response?.data?.error?.message || e.message);
    return [];
  }
}
 ...
 */
export async function replyToInstagramComment(commentId, message, accountId) {
  try {
    const token = await getInstagramAccessToken(accountId);
    const base = getGraphBase(token);
    ...
    console.warn(`[Instagram-Comments] Failed to reply to comment ${commentId}:`, e.response?.data?.error?.message || e.message);
    throw e;
  }
}
