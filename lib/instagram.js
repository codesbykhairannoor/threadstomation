...
export async function postFacebookCarousel(imageUrls, caption, fbPageId, accessToken) {
  const base = 'https://graph.facebook.com/v19.0';
  console.log(`[Facebook-Post] Uploading ${imageUrls.length} unpublished photos for carousel to page ${fbPageId}...`);
  ...
  // Using query string construction to pass array properly
  const qs = new URLSearchParams();
  qs.append('message', caption);
  qs.append('access_token', accessToken);
  attachedMedia.forEach(media => qs.append('attached_media', media));
  
  const res = await axios.post(`${base}/${fbPageId}/feed?${qs.toString()}`);
  return { publishId: res.data.id, status: 'success' };
}

export async function postToFacebook(imageUrls, caption, accountId) {
  try {
    const rows = await sql`SELECT facebook_page_id, access_token, crosspost_to_facebook FROM instagram_accounts WHERE id = ${accountId}`;
    if (!rows.length) return { skipped: true, reason: 'Account not found' };
    
    const { facebook_page_id, access_token, crosspost_to_facebook } = rows[0];
    ...
    if (!access_token || access_token.startsWith('IGA')) {
      console.log(`[Facebook-Post] Token is Instagram-only (IGA). User needs to reconnect with Facebook permissions.`);
      return { skipped: true, reason: 'Invalid token type' };
    }

    if (Array.isArray(imageUrls) && imageUrls.length > 1) {
      return await postFacebookCarousel(imageUrls, caption, facebook_page_id, access_token);
    } else {
      const url = Array.isArray(imageUrls) ? imageUrls[0] : imageUrls;
      return await postFacebookSingleImage(url, caption, facebook_page_id, access_token);
    }
  } catch (e) {
    const errData = e.response?.data?.error || {};
    const msg = errData.message || e.message;
