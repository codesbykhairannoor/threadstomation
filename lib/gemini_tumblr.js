...
`;
  ...
  if (useImage) {
    systemPrompt += `
NOTE: This post MUST include exactly 1 image. You must write a highly detailed 'full_image_prompt' that will be fed to an AI image generator.
CRITICAL RULE FOR IMAGES: The image must be PHOTOGRAPHIC or 3D RENDER only. ABSOLUTELY NO TEXT, NO WORDS, NO LETTERS in the image. The image should visually represent the concept or brand being promoted. For example: for automation/Make.com use "abstract glowing digital workflow network, purple and white, cinematic lighting, 8k". For Wise use "world map with glowing transfer lines connecting cities, gold and green, photorealistic". For productivity use "focused person at a minimalist desk with soft morning light".
INTELLIGENCE CHECK: If the topic mentions a specific brand (like Wise, Make.com, Systeme, etc.), use imagery that represents their visual identity or concept. Keep it photorealistic, cinematic, and 100% text-free.
Block info: Account ${accountName}

OUTPUT FORMAT (strict JSON, no markdown):
{
  "full_image_prompt": "A photorealistic, cinematic image representing [CONCEPT]. NO TEXT. NO WORDS. Hyperdetailed, 8k, dramatic lighting.",
  ${captionPrompt},
...
`;
  ...
  const fallbackObj = {
    "full_image_prompt": `A photorealistic, cinematic promotional image. NO TEXT, NO WORDS, NO LETTERS. Abstract glowing digital network or professional workspace, hyperdetailed, 8k, dramatic lighting.`,
