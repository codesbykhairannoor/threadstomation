...
async function fetchAIForegroundAsset(prompt, bgColorName = 'pure white') {
  ...
  // 3. Ultimate Free Fallback to Pollinations AI
  if (!assetBuffer) {
    console.log(`[AI-Asset] FALLING BACK to Pollinations AI (Free API)...`);
    try {
      const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(finalPrompt)}?width=1024&height=1024&nologo=true`;
      const res = await fetch(url);
      if (res.ok) {
        assetBuffer = Buffer.from(await res.arrayBuffer());
        console.log(`[AI-Asset] ✅ Pollinations AI SUCCESS`);
      }
    } catch (e) {
      console.warn(`[AI-Asset] ❌ Pollinations AI failed:`, e.message);
    }
  }
...
export async function generateNativeBannerImage(prompt, overlayText = null, palette = null) {
  ...
  // 3. Ultimate Free Fallback to Pollinations AI
  if (!assetBuffer) {
    console.log(`[Native-AI] FALLING BACK to Pollinations AI (Free API)...`);
    try {
      const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1024&height=1024&nologo=true`;
      const res = await fetch(url);
      if (res.ok) {
        assetBuffer = Buffer.from(await res.arrayBuffer());
        console.log(`[Native-AI] ✅ Pollinations AI SUCCESS`);
      }
    } catch (e) {
      console.warn(`[Native-AI] ❌ Pollinations AI failed:`, e.message);
    }
  }
