...
/**
 * Fetch AI Foreground Subject using Alibaba Cloud (primary) then SiliconFlow (fallback)
 */
async function fetchAIForegroundAsset(prompt, theme = null, layoutType = 'CenterMockup') {
  if (!prompt || prompt.trim() === '') return null;

  // STRICT: For ALL standard layouts, inject anti-text negative suffix
  // This prevents AI from hallucinating text/numbers/labels on screens
  const ANTI_TEXT_SUFFIX = ', no text, no words, no letters, no numbers, no labels, no logos, no typography, no writing, no captions, blank clean screen if any display shown, photorealistic, highly detailed, studio lighting';

  let finalPrompt = prompt;
  if (layoutType !== 'PureAI') {
    const bgColorName = getColorDescription(theme?.bg1);
    finalPrompt = `${prompt}${ANTI_TEXT_SUFFIX}, isolated on a ${bgColorName} background`;
  } else {
  ...
  // 2. Fallback to SiliconFlow if Alibaba failed
  if (!assetUrlOrBlob) {
...
export async function generateInstagramSlideImages(slides, customPalette = null, accountName = "@sharesa.space") {
  ...
  // Premium dark palettes — all have dark backgrounds, neon accents
  const palettes = [
    { bg1: '#042f2e', bg2: '#042f2e', accent: '#00ff8c', text: '#ffffff' }, // Dark Teal + Neon Green (original best)
    { bg1: '#1a1a2e', bg2: '#1a1a2e', accent: '#e94560', text: '#ffffff' }, // Deep Navy + Neon Red
    { bg1: '#0a1628', bg2: '#0a1628', accent: '#f7971e', text: '#ffffff' }, // Midnight Blue + Neon Orange
    { bg1: '#0f2027', bg2: '#0f2027', accent: '#fde047', text: '#ffffff' }, // Deep Teal + Gold
    { bg1: '#1a0533', bg2: '#1a0533', accent: '#a855f7', text: '#ffffff' }, // Deep Purple + Neon Purple
  ];
