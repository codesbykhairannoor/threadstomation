...
/**
 * Render SVG to PNG buffer via resvg-js
 */
...
export async function generateInstagramSlideImages(slides) {
  ...
  const promises = slides.map(async (slide, i) => {
    ...
    // 1. Generate full SVG directly (0 API calls, 0 errors!)
    ...
    const fileName = `instagram/${Date.now()}-${Math.floor(Math.random()*1000)}-slide-${i + 1}.png`;
