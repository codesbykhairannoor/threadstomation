...
/**
 * Generate AI Background Image using Pollinations
 */
async function fetchPollinationsImage(prompt) {
  ...
  console.log('[Instagram-Carousel] Fetching background from Pollinations:', url);
...
export async function generateInstagramSlideImages(slides) {
  ...
  console.log(`[Instagram-Carousel] Generating ${total} slides in parallel...`);

  const promises = slides.map(async (slide, i) => {
    console.log(`[Instagram-Carousel] Starting slide ${i + 1}/${total}...`);
    // 1. Fetch AI Background
    const bgBuffer = await fetchPollinationsImage(slide.image_prompt);
