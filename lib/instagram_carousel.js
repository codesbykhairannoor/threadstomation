...
import { renderSlideToBuffer, renderReelTextOverlayBuffer } from './layout_engine.js';
...
/**
 * Generate full-screen 9:16 (1080x1920) transparent PNG overlay buffers for Reels video.
 */
export async function generateReelTextOverlayBuffers(slides, accountName = "@adhlil.co") {
  const images = [];
  for (let i = 0; i < slides.length; i++) {
    const slide = slides[i];
    console.log(`[LayoutEngine] Rendering 9:16 Reels text overlay slide ${i + 1}/${slides.length}...`);
    const pngBuf = await renderReelTextOverlayBuffer(slide, accountName);
    images.push({ buffer: pngBuf, isRawBuffer: true });
  }
  return images;
}
