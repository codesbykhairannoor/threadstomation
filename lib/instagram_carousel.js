...
const SF_API_KEY = process.env.SILICONFLOW_API_KEY || 'sk-dpqgfpxsutmsdawhmqevcozsssfavqwbpcwvixdkeyxgsmxl';
...
/**
 * Fetch a stunning background image from SiliconFlow (FLUX model)
 */
async function fetchSiliconFlowBackground(imagePrompt) {
  ...
  console.log(`[SF] Requesting background: "${finalPrompt.substring(0, 50)}..."`);
  
  // Try fast/cheap model first, fallback to flex
  const models = ['black-forest-labs/FLUX.1-schnell', 'black-forest-labs/FLUX.2-flex'];
    ...
    try {
      const res = await fetch("https://api.siliconflow.com/v1/images/generations", {
        ...
        body: JSON.stringify({
          model: model,
          prompt: finalPrompt,
          image_size: "1024x1024", // We will crop/resize via sharp later
        })
      });
      ...
      if (res.ok) {
        ...
        if (data.images && data.images[0] && data.images[0].url) {
          ...
          // Resize and crop it to our 1080x1350 canvas
          return await sharp(buf).resize(W, H, { fit: 'cover' }).jpeg({ quality: 90 }).toBuffer();
        }
      }
      console.warn(`[SF] Model ${model} failed with ${res.status}`);
    } catch(e) {
      console.warn(`[SF] Model ${model} error:`, e.message);
    }
  }
  
  throw new Error("Failed to generate AI background from SiliconFlow after all model attempts.");
}
...
export async function generateInstagramSlideImages(slides) {
  ...
  const promises = slides.map(async (slide, i) => {
    ...
    // 1. Fetch AI Background from SiliconFlow (parallel mapping helps speed this up)
    const bgBuffer = await fetchSiliconFlowBackground(slide.image_prompt);
