...
/**
 * Helper to fetch random high-quality stock photo from Picsum Photos
 */
async function fetchFromPicsum(width = 1024, height = 1024) {
  try {
    console.log(`[Picsum-Stock] Fetching random stock photo (${width}x${height})...`);
    const res = await fetch(`https://picsum.photos/${width}/${height}`);
    if (res.ok) {
      console.log(`[Picsum-Stock] ✅ Stock photo fetched: ${res.url}`);
      return Buffer.from(await res.arrayBuffer());
    }
  } catch (err) {
    console.error(`[Picsum-Stock] Failed to fetch stock photo:`, err.message);
  }
  return null;
}
...
async function fetchAIForegroundAsset(prompt, bgColorName = 'pure white') {
  ...
  // 3. Fallback to AI Horde (Anonymous Mode)
  if (!assetBuffer) {
    console.log(`[AI-Asset] FALLING BACK to AI Horde (Anonymous)...`);
    assetBuffer = await fetchFromAIHorde(finalPrompt, 1024, 1024);
  }

  // 4. Fallback to Picsum Photos (Stock Image)
  if (!assetBuffer) {
    console.log(`[AI-Asset] FALLING BACK to Picsum Photos Stock Image...`);
    assetBuffer = await fetchFromPicsum(1024, 1024);
  }
...
export async function generateNativeBannerImage(prompt) {
  ...
  // 3. Fallback to AI Horde (Anonymous Mode)
  if (!assetBuffer) {
    console.log(`[Native-AI] FALLING BACK to AI Horde (Anonymous)...`);
    assetBuffer = await fetchFromAIHorde(prompt, 1024, 1024);
  }

  // 4. Fallback to Picsum Photos (Stock Image)
  if (!assetBuffer) {
    console.log(`[Native-AI] FALLING BACK to Picsum Photos Stock Image...`);
    assetBuffer = await fetchFromPicsum(1024, 1024);
  }
