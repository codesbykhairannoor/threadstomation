...
function buildSlideSvg(slideIndex, totalSlides, title, body, themeIndex, layoutType) {
  ...
  if (layoutType === 0) {
    ...
    const TITLE_Y = 780;
    const titleLines = wrapText(titleText, 22);
    const bodyLines = wrapText(bodyText, 36);
    const BODY_Y = TITLE_Y + (titleLines.length * 85) + 50;

    const titleSvg = titleLines.map((line, i) => 
      `<text x="120" y="${TITLE_Y + i * 85}" font-family="system-ui, -apple-system, sans-serif" font-size="60" fill="${palette.accent}" font-weight="900" text-anchor="start">${escapeXml(line)}</text>`
    ).join('');

    const bodySvg = bodyLines.map((line, i) => 
      `<text x="120" y="${BODY_Y + i * 65}" font-family="Georgia, 'Times New Roman', serif" font-size="40" fill="white" font-weight="normal" text-anchor="start" opacity="0.9">${escapeXml(line)}</text>`
    ).join('');
