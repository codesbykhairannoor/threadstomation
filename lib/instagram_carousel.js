...
export async function generateInstagramSlideImages(slides, customPalette = null, accountName = "@sharesa.space") {
  ...
  // SHARESA.SPACE WHITE/LIGHT BRAND COLORS
  const SHARES_DARK = '#1e2a39';
  const SHARES_GREEN = '#00ff8c';
  const SHARES_WHITE = '#ffffff';
  const SHARES_OFF_WHITE = '#f8fafc';

  // We enforce a pure white background for all layouts so that the AI generated images
  // (which have pure white backgrounds) blend seamlessly without needing buggy background removal APIs.
  const palettes = [
    { bg1: SHARES_WHITE, bg2: SHARES_WHITE, accent: SHARES_GREEN, text: SHARES_DARK }, // Pure White + Neon Green
    { bg1: SHARES_OFF_WHITE, bg2: SHARES_OFF_WHITE, accent: SHARES_GREEN, text: SHARES_DARK }, // Off White + Neon Green
  ];
  ...
  if (parsedPalette) {
    ...
    // FORCE white background logic for AI image blending, no matter what user settings say.
    // This is the clean, definitive fix for the ugly box problem.
    theme.bg1 = '#ffffff';
    theme.bg2 = '#ffffff';
    theme.text = '#1e2a39'; // Dark text
    theme.accent = '#00ff8c'; // Green accent
  } else {
