...
export async function generateInstagramSlideImages(slides, customPalette = null, accountName = "@sharesa.space") {
  ...
  // We now alternate between Light Mode and Dark Mode per carousel generation.
  // The layout engine handles the AI image gracefully by putting it inside a White Capsule Widget on Dark mode.
  const palettes = [
  ...
  if (parsedPalette) {
    ...
    theme = useDark ? palettes[1] : palettes[0];
  }
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    if (slide.foreground_subject_prompt && slide.layout_type !== 'TextHeavy') {
      try {
        const bgColorName = theme.name === 'dark' ? 'dark navy blue' : 'pure white';
        fgBuffer = await fetchAIForegroundAsset(slide.foreground_subject_prompt, bgColorName);
      } catch (e) {
