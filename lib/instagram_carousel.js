 ...
 */
async function removeBackgroundHF(imageBuffer) {
  if (!process.env.HF_TOKEN) {
    console.warn("[BG-Removal] HF_TOKEN not found, skipping background removal.");
    return imageBuffer;
  }
  try {
    console.log("[BG-Removal] Sending to Hugging Face briaai/RMBG-1.4...");
    ...
    if (!response.ok) {
      console.warn(`[BG-Removal] API Error: ${response.status} - skipping.`);
      return imageBuffer;
    }
    ...
    console.warn("[BG-Removal] Network error, bypassing:", e.message);
    return imageBuffer;
  }
}
...
export async function generateInstagramSlideImages(slides, customPalette = null, accountName = "@sharesa.space") {
  ...
  // 3 Dynamic Palettes for cyclic rotation
  const palettes = [
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    // Select theme dynamically: use customPalette if provided, else cycle through the 3 brand tones
    const theme = customPalette || palettes[i % palettes.length];
    
    // 1. Generate Asset if needed
    let fgBuffer = null;
    if (slide.foreground_subject_prompt && slide.layout_type !== 'TextHeavy') {
      try {
        const rawBuffer = await fetchAIForegroundAsset(slide.foreground_subject_prompt);
        fgBuffer = await removeBackgroundHF(rawBuffer);
      } catch (e) {
