...
function detectLanguage(text) {
  const idWords = ['yang', 'dan', 'di', 'ini', 'itu', 'kamu', 'gue', 'gw', 'lu', 'kita', 'adalah', 'dengan', 'untuk', 'tidak', 'bisa', 'mau', 'ada', 'saya', 'anda', 'cara', 'tips', 'tahu', 'buat', 'dari', 'apakah', 'tapi', 'aja', 'gitu', 'sama', 'ke', 'akan', 'lebih', 'bukan', 'hanya', 'harus', 'kalau', 'bahkan', 'banyak', 'orang', 'kerja', 'hasilnya'];
 ...
 * ALWAYS detects language from the actual spoken script first.
 */
...
export function buildVoiceoverScript(slides, caption = '') {
  ...
  return parts
    ...
    .filter(Boolean)
    .join('. ')
    .replace(/([.?!])\s*[\.?!]+/g, '$1')  // Fix double punctuation like ?!. or ..
    .replace(/\s+/g, ' ');
}
