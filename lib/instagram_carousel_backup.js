...
function buildSlideSvg({ title, body, slideIndex, totalSlides, layoutType = 0, paletteOverride = null }) {
  ...
  if (totalSlides > 1) {
    ...
    dotsSvg = Array.from({ length: totalSlides }, (_, i) => {
  ...
  if (layoutType === 0) {
    ...
    const TITLE_Y = bodyText ? 400 : 540;
    ...
    svgElements = `
      ...
      ${titleSvg}
      ${bodyText ? `<circle cx="540" cy="${BODY_Y - 50}" r="6" fill="${palette.accent}" opacity="0.8"/>` : ''}
      ${bodySvg}
    `;
  } else if (layoutType === 1) {
    // LAYOUT 1: LEFT-ALIGNED EDITORIAL
    const TITLE_Y = bodyText ? 350 : 480;
    ...
    const barHeight = (titleLines.length * 95) + (bodyLines.length * 70) + (bodyText ? 20 : -40);
    ...
    const TITLE_Y = bodyText ? 380 : 560;
    ...
    const customDots = totalSlides > 1 ? Array.from({ length: totalSlides }, (_, i) => {
    ...
    svgElements = `
      ...
      ${totalSlides > 1 ? `<!-- Slide Number right aligned -->
      ...
      <!-- Pill Tag (Category) -->
      <rect x="140" y="${TITLE_Y - 140}" width="220" height="50" rx="25" fill="${palette.accent}" opacity="0.15" />
      <text x="250" y="${TITLE_Y - 105}" font-family="system-ui, -apple-system, sans-serif" font-size="20" fill="${palette.accent}" font-weight="800" text-anchor="middle" letter-spacing="2">INSIGHT</text>
