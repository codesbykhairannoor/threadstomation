...
- DILARANG KERAS menggunakan kata "lo"! Gunakan kata inklusif "kita" atau langsung sebut poinnya.
...
export async function runEngagementSeeder() {
  ...
  try {
    // 1. Get recent ROOT posts from the last 12 hours that haven't been seeded yet
    ...
    const recentPosts = await sql`
      ...
      WHERE platform = 'threads' 
        ...
        AND status = 'success'
        AND (is_thread_child IS NULL OR is_thread_child = FALSE)
    ...
    if (!recentPosts.length) {
      console.log('[Engagement-Seeder] 💤 No fresh root posts in the last 12 hours. Nothing to seed.');
      return { seeded: 0 };
    }
    ...
    for (const post of recentPosts) {
      // 🚨 CRITICAL SAFETY: Never seed thread continuation parts (e.g. 2/3, 3/3, 4/4)
      const content = (post.content || '').trim();
      const isChild = post.is_thread_child === true || (/[2-9]\/[0-9]/.test(content) && !/1\/[0-9]/.test(content));
      if (isChild) {
        continue;
      }

      // 🚨 STRICT SINGLE-SEED CAP: Has this post ALREADY been seeded by ANY helper account?
      // Exactly 1 single helper seed comment is allowed per post. Never accumulate 2+ helper comments!
      const [alreadySeededAny] = await sql`
        SELECT comment_id FROM replied_comments 
        WHERE parent_media_id = ${post.threads_id} AND comment_author = 'cross_seed'
      `.catch(() => [null]);

      if (alreadySeededAny) {
        // Already seeded by a helper. Skip to prevent double comments or artificial bot pile-ons!
        continue;
      }
      ...
      // Pick one random helper account to vary who supports whom naturally
      const helper = helperAccounts[Math.floor(Math.random() * helperAccounts.length)];
      if (!helper) continue;

      console.log(`[Engagement-Seeder] 🌱 Seeding ROOT post ${post.threads_id} (Author ID: ${post.account_id}) using Helper @${helper.name} (ID: ${helper.id})...`);
