...
export async function runEngagementSeeder() {
  ...
  try {
    // 1. Get recent posts from the last 12 hours that haven't been seeded yet
    ...
    if (!recentPosts.length) {
      console.log('[Engagement-Seeder] 💤 No fresh posts in the last 12 hours. Nothing to seed.');
      return { seeded: 0 };
    }
    ...
    for (const post of recentPosts) {
      ...
      // Pick one helper account that hasn't seeded this post yet
      let chosenHelper = null;
      for (const helper of helperAccounts) {
        const [alreadySeeded] = await sql`
          SELECT comment_id FROM replied_comments 
          WHERE parent_media_id = ${post.threads_id} AND account_id = ${helper.id} AND comment_author = 'cross_seed'
        `.catch(() => [null]);
      ...
      if (!chosenHelper) continue;
      const helper = chosenHelper;
