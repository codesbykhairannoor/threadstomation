...
3. Language: ${isIndo ? 'Natural conversational Indonesian (Gunakan kata ganti inklusif "kita" atau "kamu". DILARANG KERAS menggunakan kata "lo"!)' : 'Natural conversational English'}.
...
`;
  ...
  if (isIndo) {
    return 'Mantap banget sudut pandangnya! Setuju banget sama poin ini 🙌';
  }
...
async function processThreadsComments() {
  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      // Skip continuation thread parts (e.g. 2/3, 3/3, 4/4)
      ...
      for (const reply of replies) {
        ...
        // 🚨 2. STRICT: NEVER reply to our own network/helper handles!
        // This eliminates nested sub-replies ("replay di replay") and fake bot-on-bot loop cascades.
        // Helper accounts are ONLY meant to leave 1 seed comment to trigger Meta's discovery pool.
  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      for (const comment of comments) {
        ...
        // 🚨 2. STRICT: NEVER reply to our own network/helper handles on Instagram!
        if (OWN_SYSTEM_HANDLES.has(authorUsername)) {
          continue;
        }
