...
import { fetchRecentReplies, replyToThread } from './threads.js';
import { fetchInstagramComments, replyToInstagramComment } from './instagram.js';
  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      for (const reply of replies) {
        ...
        console.log(`[AI-Replier] 📩 Found new Threads comment from @${reply.username || 'user'}: "${reply.text.slice(0, 60)}..."`);
  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      for (const comment of comments) {
        ...
        console.log(`[AI-Replier] 📩 Found new Instagram comment from @${comment.username || 'user'}: "${comment.text.slice(0, 60)}..."`);
        ...
        if (replyText) {
          try {
            ...
            // Record as skipped to avoid re-attempting invalid comment/permission errors endlessly
            await sql`
              INSERT INTO replied_comments (comment_id, platform, account_id, parent_media_id, comment_author, comment_text, reply_text)
...
export async function runCommentReplier() {
  ...
  if (total > 0) {
    ...
    console.log(`[AI-Replier] 💤 No pending unreplied comments found. Everything is up-to-date.`);
  }
