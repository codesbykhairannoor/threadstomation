  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      for (const reply of replies) {
        ...
        // 🚨 CRITICAL SELF-REPLY & LOOP GUARD:
        // 1. NEVER reply to ourselves (prevents bot replying to itself in an infinite loop)!
        // 2. NEVER reply to any account in our network (prevents automated bot-to-bot loops)!
        if (authorUsername === myUsername || OWN_SYSTEM_HANDLES.has(authorUsername)) {
          // Record as SKIPPED_SELF so it's marked as processed and ignored forever
          await sql`
            INSERT INTO replied_comments (comment_id, platform, account_id, parent_media_id, comment_author, comment_text, reply_text)
            VALUES (${reply.id}, 'threads', ${post.account_id}, ${post.threads_id}, ${reply.username || ''}, ${reply.text}, 'SKIPPED_SELF')
  ...
  try {
    ...
    for (const post of recentPosts) {
      ...
      for (const comment of comments) {
        ...
        // 🚨 CRITICAL SELF-REPLY & LOOP GUARD:
        if (authorUsername === myIgUsername || OWN_SYSTEM_HANDLES.has(authorUsername)) {
          await sql`
            INSERT INTO replied_comments (comment_id, platform, account_id, parent_media_id, comment_author, comment_text, reply_text)
            VALUES (${comment.id}, 'instagram', ${post.account_id}, ${post.creation_id}, ${comment.username || ''}, ${comment.text}, 'SKIPPED_SELF')
            ON CONFLICT (comment_id) DO NOTHING
          `.catch(() => {});
          continue;
        }
