...
export async function getFacebookAccessToken(accountId) {
  const rows = await sql`
    SELECT access_token FROM facebook_accounts WHERE id = ${accountId}
  `;
...
export async function runFacebookCarouselPost(accountId, customPrompt = null) {
  ...
  const result = await postMultiPhotoToFacebook(acc.facebook_page_id, acc.access_token, uploadedUrls, caption);
