...
OUTPUT FORMAT (strict JSON, no markdown):
{
  "slides": [
    { 
      ...
      "spoken_narration": "REQUIRED: 1 natural, conversational spoken sentence (10-20 words) for AI voiceover. Sound like a real creator talking directly to the audience! E.g. 'Pernah gak sih ngerasa kerja seharian tapi hasilnya gitu-gitu aja?'",
...
- HUMANIZED CONVERSATIONAL TONE (CRITICAL):
  - Use casual, natural, energetic spoken Indonesian (gaya bicara kreator Reels/TikTok yang akrab & engaging).
  - DO NOT sound like a robot, newspaper, or textbook. Avoid stiff phrases like "pertanyaannya adalah", "pada hakikatnya", "adapun".
  - Use natural conversational openers: "Pernah gak sih...", "Gini triknya...", "Nah, kuncinya...", "Biar gak boncos...", "Coba lakuin ini...".
...
${isCaridisini ? "- BRAND RULE: Every slide MUST feature high-quality realistic photography or realistic product/concept render. You MUST NOT use vectors, drawings, cartoons, or abstract graphics. The foreground_subject_prompt must clearly, physically, and literally show the product or concept being promoted (e.g. 'A close-up of a smartphone screen showing a secure payment success mark' or 'A neat stack of money next to a passport'). It must be crystal clear and realistic." : ""}
${isTranvas || isCaridisini ? "- BRAND RULE: The cover slide (Slide 1) MUST NOT use the 'TextHeavy' layout type. It MUST use an image-based layout (CenterMockup, TopMockup, LeftPerson, or RightPerson) with a high-quality foreground image prompt to act as an eye-catching cover." : ""}
${recentTopicsSection}
...
- BODY RULES (CRITICAL): ${numSlides > 1 ? 'Body MUST NOT be empty for carousel slides. Each body is REQUIRED to be exactly 1 clear, concise sentence (40-90 characters). DO NOT leave body as empty string.' : 'Body is optional for single feed posts.'}
- DYNAMIC LANGUAGE & TONE (CRITICAL): ${isTranvas || isCaridisini ? 'You MUST output 100% of the content in ENGLISH. No exceptions.' : 'Analyze the language of the persona/master prompt above. If it is in English, you MUST output 100% of the content in English. If it is in Indonesian, output in Indonesian. The tone must be perfectly aligned with the persona. If no persona is provided, default to a smart, engaging, casual tone.'}
