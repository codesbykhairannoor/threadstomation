  ...
  "slides": [
    {
      ...
      "body": "Optional body copy/features. To render checkmark capsule badges (pills) on standard layouts, format this field as a list of comma-separated items (e.g. 'Domain Gratis, Hosting Cepat, Garansi Penuh'). Limit to 2-3 features, max 3-4 words each. Leave empty '' if not needed.",
...
STRICT RULES:
- LAYOUT VARIATION (CRITICAL): You MUST NOT use the same "layout_type" twice in a row. Force variation! If slide 1 is 'CenterMockup', slide 2 MUST be 'LeftPerson' or 'TextHeavy' or 'PureAI', etc.
...
- STRICT DICTIONARY SPELLING (CRITICAL): Never use slang, double-letters, or intentional spelling typos in slide titles (e.g., do NOT write "Webssite", "MASIIH", "Bisniss", "Sihh", "biariin"). Ensure every word is perfectly spelled according to standard Indonesian (KBBI) or English dictionary.
- LOGICAL SEMANTIC ACCURACY (CRITICAL): Pay close attention to word meanings. Do NOT confuse positive outcomes with negative outcomes. E.g., if a slow website is a threat, use words like "Merosot", "Melorot", "Hilang" (e.g. "Jangan Biarkan Omzet Melorot!") instead of "Melesat" (which means shooting up/skyrocketing).
- LENGTH LIMITS (CRITICAL): Keep titles short. title_part1 must be maximum 3-4 words. title_part2 must be maximum 1-2 words. Keep body text extremely short (max 2-3 comma-separated features) or empty "".
