...
export async function generateVisualMetaphorSpec(customPrompt = null, masterPrompt = '', accountName = '@tranvas', accountId = 2) {
  ...
  const systemPrompt = `
You are an ELITE Creative Director for viral Visual Thinking Reels (EXACTLY like @mindset.therapy: clean, dark, hypnotic, hyper-minimalist).
...
Angle / Topic: "${customPrompt || 'an insightful paradox or mental model about productivity, habits, or mental clarity'}"

Choose the single BEST primitive:
1. "ChaosToPrism" (Scattered thoughts/tasks sucked into central prism, emitting 1 pure flow laser beam)
2. "DivergentBranch" (1% daily compounding curve vs flat drift)
3. "IcebergDepth" (Surface noise vs massive deep life OS bedrock)
...
OUTPUT FORMAT (strict JSON, no markdown):
{
  "primitive": "One of: ChaosToPrism, DivergentBranch, IcebergDepth, PhysicsFulcrum",
  "headline": "Ultra-punchy headline, MAX 3 TO 6 WORDS. E.g. 'One System. Zero Friction.' or 'Ideas In. Pure Flow Out.' or 'Confidence Runs Deep.'",
  "primitiveData": {
    "multiplier": 37.8
  },
    ...
    for (const modelName of GEMINI_MODELS) {
      try {
        ...
        if (!primitives.includes(parsed.primitive)) {
          parsed.primitive = 'ChaosToPrism';
        }

        if (accountId && (parsed.headline || customPrompt)) {
