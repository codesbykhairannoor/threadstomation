...
export async function renderSlideToBuffer(slide, foregroundBuffer, theme, badgeText) {
  ...
  const highlightTitle = {
    type: 'div',
    props: {
      style: { background: accentColor, padding: '12px 30px', borderRadius: '8px', marginTop: '15px', display: 'flex' },
      children: [
  ...
  if (layout === 'PureAI' && fgBase64) {
    // Pure AI Slide: The generated image occupies the ENTIRE slide background
    // Satori only overlays the header badge and bottom ribbon for branding consistency
    vdom = {
      type: 'div',
      props: {
        style: bgStyle,
        children: [
          // The full bleed AI generated image
          { type: 'img', props: { src: fgBase64, style: { position: 'absolute', top: '0', left: '0', width: '1080px', height: '1350px', objectFit: 'cover' } } },
          // Top subtle shadow overlay
          { 
            type: 'div', 
            props: { 
              style: { 
    ...
    vdom = {
      ...
      props: {
        ...
        children: [
          ...
          { type: 'img', props: { src: fgBase64, style: { position: 'absolute', bottom: '150px', right: '50px', width: '480px', height: '900px', objectFit: 'cover', borderRadius: '40px', border: `8px solid ${accentColor}` } } },
      ...
      props: {
        ...
        children: [
          ...
          { type: 'span', props: { style: { color: mainTextColor, fontSize: '110px', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-4px', textAlign: 'center' }, children: slide.title_part1 || '' } },
          {
            type: 'div',
            props: {
              style: { background: accentColor, padding: '15px 50px', borderRadius: '12px', marginTop: '30px', display: 'flex' },
              children: [
