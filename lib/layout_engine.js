...
export async function renderSlideToBuffer(slide, foregroundBuffer, theme, badgeText) {
  const isJpeg = foregroundBuffer && foregroundBuffer.length > 2 && foregroundBuffer[0] === 0xFF && foregroundBuffer[1] === 0xD8;
  const mimeType = isJpeg ? 'image/jpeg' : 'image/png';
  const fgBase64 = foregroundBuffer ? `data:${mimeType};base64,${foregroundBuffer.toString('base64')}` : null;
  ...
  // Subtle micro-jitter for unique perceptual hash (bypasses Meta duplicate template detection)
  ...
  const bgOrbs = [
    { type: 'div', props: { style: { position: 'absolute', top: `${-15 + jitterY * 0.1}%`, left: `${-10 + jitterX * 0.1}%`, width: `${800 + jitterX}px`, height: `${800 + jitterY}px`, borderRadius: '400px', background: accentColor, opacity: isLightBg ? 0 : 0.15, filter: 'blur(100px)' } } },
    { type: 'div', props: { style: { position: 'absolute', bottom: `${-20 - jitterY * 0.1}%`, right: `${-15 - jitterX * 0.1}%`, width: `${900 - jitterX}px`, height: `${900 - jitterY}px`, borderRadius: '450px', background: mainTextColor, opacity: isLightBg ? 0 : 0.08, filter: 'blur(120px)' } } }
  ];
  ...
  const watermarkText = {
    type: 'div',
    props: {
      style: { position: 'absolute', top: `${150 + jitterY}px`, left: `${-100 + jitterX}px`, fontSize: '280px', fontWeight: 900, color: isLightBg ? 'rgba(0,0,0,0.02)' : 'rgba(255,255,255,0.02)', whiteSpace: 'nowrap', zIndex: 0 },
...
/**
 * Renders a Full-Screen 9:16 (1080x1920) Transparent Text Overlay for B-Roll Reels.
 * Creates clean, modern, viral subtitle-style typography over cinematic Pexels video background.
 */
export async function renderReelTextOverlayBuffer(slide, badgeText = '@adhlil.co') {
  ...
  const vdom = {
    ...
    props: {
      style: {
        ...
        // Dark vignette gradient overlay for text contrast against light/dark B-Roll
        background: 'linear-gradient(to bottom, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.15) 30%, rgba(0,0,0,0.4) 65%, rgba(0,0,0,0.85) 100%)',
        fontFamily: 'Inter',
      },
      children: [
        // Top Header Badge
        ...
        // Center / Lower-Center Main Subtitle Content
        {
          type: 'div',
          props: {
            style: {
            ...
            children: [
              ...
              // Main Title Part 2 (Highlight Yellow Accent)
              title2 ? {
                type: 'span',
                props: {
                  style: {
            ...
            children: [
              {
                ...
                props: {
                  ...
                  children: '💡 SIMPAN & FOLLOW UNTUK TIPS HARIAN',
                },
              },
            ],
          },
  ...
  const resvg = new Resvg(svg, {
    background: 'rgba(0,0,0,0)', // Transparent PNG background!
    fitTo: { mode: 'width', value: 1080 }
  });
