...
export async function renderSlideToBuffer(slide, foregroundBuffer, theme, badgeText) {
  ...
  if (layout === 'CenterMockup' && fgBase64) {
    // CenterMockup: Full-bleed bottom image with SEAMLESS GRADIENT BLEND!
    vdom = {
      type: 'div',
      props: {
        style: bgStyle,
        children: [
          ...
          { type: 'img', props: { src: fgBase64, style: { position: 'absolute', bottom: '70px', left: '0px', width: '1080px', height: '800px', objectFit: 'cover' } } },
          // SEAMLESS FADE OVERLAY: Fades the top of the image into the solid background!
          { type: 'div', props: { style: { position: 'absolute', bottom: '500px', left: '0px', width: '1080px', height: '370px', background: `linear-gradient(to bottom, ${solidBg} 0%, rgba(0,0,0,0) 100%)` } } },
    ...
    // LeftPerson: Actually splitting the screen vertically. Image bleeds off the right edge.
    vdom = {
      type: 'div',
      props: {
        style: bgStyle,
        children: [
          ...bgOrbs,
          // Image on the right bleeding out
          { type: 'img', props: { src: fgBase64, style: { position: 'absolute', bottom: '70px', right: '0px', width: '600px', height: '1000px', objectFit: 'cover', borderRadius: '40px 0 0 0', boxShadow: '-20px 0 50px rgba(0,0,0,0.5)' } } },
          // Horizontal fade to blend the left edge of the image
          { type: 'div', props: { style: { position: 'absolute', bottom: '70px', right: '400px', width: '250px', height: '1000px', background: `linear-gradient(to right, ${solidBg} 0%, rgba(0,0,0,0) 100%)`, zIndex: 2 } } },
