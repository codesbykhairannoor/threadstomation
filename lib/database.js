...
import { cleanupOldStorage } from './supabase_storage.js';

dotenv.config({ override: true });

// Convert pooler port (6543) to direct connection port (5432) to prevent timeout
const connectionString = process.env.DATABASE_URL ? process.env.DATABASE_URL.replace(':6543', ':5432') : null;
let sql;
if (connectionString) {
  ...
  console.warn('WARNING: DATABASE_URL not set. Stubbing database operations to return empty results.');
...
const _doInit = async () => {
  try {
    ...
    try {
      ...
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN facebook_access_token TEXT`; } catch (e) {}
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN crosspost_to_facebook INTEGER DEFAULT 1`; } catch (e) {}
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN master_prompt TEXT`; } catch (e) {}
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN visual_theme TEXT`; } catch (e) {}
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN color_palette TEXT`; } catch (e) {}
      try { await sql`ALTER TABLE instagram_accounts ADD COLUMN preferred_layout INTEGER DEFAULT 0`; } catch (e) {}
      await sql`
    ...
    try {
      ...
      // Threads thread continuation tracking (prevents seeding non-root parts)
      await sql`ALTER TABLE post_history ADD COLUMN IF NOT EXISTS is_thread_child BOOLEAN DEFAULT FALSE`;
      await sql`UPDATE post_history SET is_thread_child = TRUE WHERE content ~ '^[2-9]/[0-9]'`;
    } catch (migErr) {
...
export async function cleanupOldHistory() {
  try {
    // Aggressive cleanup: 1 Day (24 hours) to keep DB super lightweight
    const oneDayAgo = new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString();

    await sql`DELETE FROM post_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM tiktok_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM instagram_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM facebook_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM tumblr_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM devto_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM mastodon_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM bluesky_history WHERE created_at < ${oneDayAgo}`;
    await sql`DELETE FROM topic_history WHERE created_at < ${oneDayAgo}`;

    console.log('[DB-Cleanup] Aggressively cleaned up local database history older than 1 day.');
    
    console.log('[DB-Cleanup] Initiating Supabase Storage cleanup...');
    await cleanupOldStorage();
  } catch (e) {
    console.error('[DB-Cleanup] Failed to cleanup history or storage:', e.message);
  }
}
