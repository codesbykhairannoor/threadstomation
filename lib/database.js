...
const _doInit = async () => {
  try {
    ...
    // ── TIKTOK TABLES (Must run unconditionally because they were added later) ──
    try {
      await sql`
      ...
      console.log('[DB] Note: TikTok tables initialization note:', tiktokDbErr.message);
    }
    ...
    try {
      ...
      console.log('[DB] Note: Instagram tables initialization note:', instagramDbErr.message);
    }
...
/**
 * Cleans up history tables to prevent database bloat.
 * Deletes records older than 2 days. This DOES NOT delete posts on the actual platforms.
 */
