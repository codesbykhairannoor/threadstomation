...
export async function getThreadsAccessToken() {
  const row = db.prepare('SELECT access_token, expires_at FROM tokens WHERE id = 1').get();
  if (!row || !row.access_token) {
    throw new Error('Threads account not linked. Please Link Account first.');
  }

  // Token longevity check could go here
...
export async function refreshThreadsToken(oldToken) {
  try {
    ...
    db.prepare('UPDATE tokens SET access_token = ?, expires_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1')
