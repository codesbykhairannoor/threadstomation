...
export async function postToThreadsOfficial(content, imageUrl = null, accountId = 1, replyToId = null) {
  try {
    ...
    if (error.response?.status === 401 || errorData.code === 190) {
      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          return await postToThreadsOfficial(content, imageUrl, accountId, replyToId); 
        }
      } catch (refreshErr) {
...
/**
 * Fetches recent published Threads posts for an account
 */
export async function fetchRecentThreadsPosts(accountId = 1, limit = 5) {
  try {
    const accessToken = await getThreadsAccessToken(accountId);
    const res = await axios.get(`${GRAPH_BASE_URL}/me/threads`, {
      params: {
    ...
    console.warn(`[Threads-Acc:${accountId}] Failed to fetch recent posts:`, err.response?.data?.error?.message || err.message);
    return [];
  }
}

/**
 * Fetches replies/comments on a specific Threads post
 */
export async function fetchRecentReplies(mediaId, accountId = 1) {
  try {
    const accessToken = await getThreadsAccessToken(accountId);
    const res = await axios.get(`${GRAPH_BASE_URL}/${mediaId}/replies`, {
      params: {
    ...
    console.warn(`[Threads-Acc:${accountId}] Failed to fetch replies for ${mediaId}:`, err.response?.data?.error?.message || err.message);
    return [];
  }
}

/**
 * Posts a reply to an existing Threads post
 */
export async function replyToThread(mediaId, replyText, accountId = 1) {
  return await postToThreadsOfficial(replyText, null, accountId, mediaId);
}
