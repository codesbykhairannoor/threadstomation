...
export async function postTikTokCarousel(imageUrls, caption, hashtags = [], accountId) {
  ...
  // Check if sandbox mode is enabled in database
  const sandboxSetting = await sql`SELECT value FROM tiktok_settings WHERE key = 'tiktok_sandbox_mode'`;
  const isSandbox = sandboxSetting[0]?.value === 'true';

  let privacyLevel;
  if (isSandbox) {
    privacyLevel = 'SELF_ONLY';
  } else {
    privacyLevel = privacyOptions.includes('PUBLIC_TO_EVERYONE')
      ? 'PUBLIC_TO_EVERYONE'
      : privacyOptions[0];
  }
