...
export async function generateShopeeAffiliatePost(product, accountId = 1) {
  ...
  const prompt = `
    ...
    - Format: Start with a hook, mention the price/benefits, end with CTA and the link.
    
    STRICT RULES:
    - DO NOT use hashtags.
    - ABSOLUTE CHARACTER LIMIT: 450 characters including the link.
  ...
  for (const modelName of modelNames) {
    try {
      console.log(`[Gemini-Shopee] Attempting ${modelName}...`);
      const model = genAI.getGenerativeModel({ model: modelName });
      ...
      console.log(`[Gemini-Shopee] ✅ Generated ${text.length} chars using ${modelName}`);
      return text;
    } catch (e) {
      console.warn(`[Gemini-Shopee] Model ${modelName} failed: ${e.message.split('\n')[0]}`);
      lastError = e;
      continue;
    }
  }

  throw new Error(`Gemini-Shopee failed all model attempts: ${lastError?.message}`);
}
