...
Output ONLY the final post content.`;
      ...
      // If it's a 503 (High demand), wait 2s and try one more time for this model
      if (e.message.includes('503') || e.message.includes('Service Unavailable')) {
        ...
        try {
          // Retry logic (duplicated for simplicity in this specific block)
          const model = genAI.getGenerativeModel({ model: modelName });
          const result = imageData ? await model.generateContent([promptTextBase, { inlineData: { data: imageData.split(',')[1] || imageData, mimeType: "image/jpeg" } }]) : await model.generateContent(platformPrompt);
          const response = await result.response;
          return response.text().trim();
        } catch (retryErr) {
          console.warn(`[Gemini] Retry for ${modelName} also failed:`, retryErr.message);
        }
      }
