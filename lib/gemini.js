  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    for (const modelName of geminiModels) {
      try {
        ...
        // Skenario 503 (Busy) atau 429 (Rate Limit): Sabar sebentar terus coba lagi
        if (msg.includes('503') || msg.includes('429') || msg.includes('quota')) {
          const isQuota = msg.includes('429') || msg.includes('quota');
          console.warn(`[Gemini-Shopee] ${isQuota ? 'Quota Limit' : 'Model Busy'}. Waiting 5s before retry...`);
          ...
          try {
            ...
            console.warn(`[Gemini-Shopee] Retry failed for ${modelName}: ${retryErr.message.split('\n')[0]}`);
          }
        }
