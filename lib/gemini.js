...
- PSYCHOLOGICAL STRUCTURE (Strictly 2 to 4 posts):
  1. POST 1: Brutal, relatable PAIN POINT. The agony of "app fatigue" (juggling 10 apps: Notion, Keep, Apple Notes, WA), messy to-do lists, 15 open tabs, zero real progress. An opening hook that hurts because it's true.
  2. POST 2 (and 3): AGITATE the cost of staying disorganized (lost time, missed opportunities, mental exhaustion, goals resetting every year) + REVEAL the GAIN of an organized Second Brain (wake up with 100% clarity, 1 sleek Notion dashboard, zero brain fog, 3x deep work).
  3. FINAL POST (Post 3 or 4): POSITION TRANVAS LIFE-OS BLUEPRINT as the immediate, no-brainer SOLUTION! End with a direct, urgent HARD-SELL CTA: "Stop buang-buang waktu dalam kekacauan. Ambil kontrol hidup lo sekarang. Klik link di bio buat akses Tranvas Life-OS Blueprint sekarang juga!"
`;
  } else if (isSharesa) {
    hardSellingRules = `
...
- PSYCHOLOGICAL STRUCTURE (Strictly 2 to 4 posts):
  1. POST 1: Brutal, relatable PAIN POINT. Expose the trap of selling ONLY via Instagram DM/WhatsApp or paying 10-15%+ marketplace cut fees ("numpang di tanah orang"), losing big clients because of having no professional website.
  2. POST 2 (and 3): AGITATE the fatal risk (account banned, algorithm drops, zero database ownership) + REVEAL the GAIN of owning a "Rumah Digital" (website) that sells 24/7 on autopilot, instant 10x corporate credibility, 100% profit margins.
  3. FINAL POST (Post 3 or 4): POSITION SHARESA SPACE (@sharesa.space) as the immediate SOLUTION to build their high-converting website! End with a direct, urgent HARD-SELL CTA: "Saatnya bisnis lo punya Rumah Digital sendiri yang hasilin cuan 24/7. Slot pembuatan web terbatas! Konsultasi gratis sekarang lewat link di bio!"
`;
  }
