...
async function uploadVideoToSupabase(buffer) {
  // Use a dynamic import to reuse the Supabase client logic from the existing file
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.error('[Supabase] Missing credentials for video upload.');
    return null;
  }

  const supabase = createClient(supabaseUrl, supabaseKey);
  const fileName = `reels/reel-${Date.now()}-${Math.floor(Math.random() * 1000)}.mp4`;

  console.log(`[VideoGenerator] Uploading video to Supabase: ${fileName}`);
  const { error } = await supabase.storage
    .from('media') // Same public bucket
  ...
  if (error) {
    console.error('[VideoGenerator] Supabase upload failed:', error.message);
