 ...
 * @param {Array<object>} [slides] - Optional Gemini slide objects for Pexels keyword extraction.
 * @param {string} [masterPrompt] - Account persona for fallback keyword extraction.
 * @returns {Promise<string>} The public URL of the uploaded MP4 video.
 */
export async function createVideoFromImages(imageBuffers, durationPerSlide = 3, slides = [], masterPrompt = '') {
  return new Promise(async (resolve, reject) => {
    ...
    if (brollPath) {
       ...
       * Input 2: BGM MP3 (optional)
       *
       * Filter:
       * [0:v] Scale + loop B-Roll to fill total video duration, force 9:16 → [bg]
       * [1:v] Scale overlay images to 85% width, center them → [fg]
       * [bg][fg] overlay at center with slight vertical offset for visual hierarchy → [out]
       *
       * The slide overlay has a soft drop shadow baked in via a fast box-blur trick:
       * pad the fg by 10px on all sides, fill with semi-black, then overlay.
       */
      const filterComplex = [
      ...
      cmd
        ...
        .on('error', async (err) => {
          ...
          await createStaticSlideshow(slidePaths, bgmPath, durationPerSlide, outputVideoPath, tempFiles, resolve, reject, sessionId);
        });
