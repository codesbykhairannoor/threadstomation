...
async function uploadVideoToSupabase(buffer) {
  try {
    ...
    console.log(`[VideoGenerator] Bypassing Supabase, uploading video to Uguu.se: ${fileName}`);
    ...
    if (!res.ok) {
      ...
      throw new Error(`Uguu upload failed: ${errorText}`);
    }

    const data = await res.json();
    if (!data.success || !data.files || data.files.length === 0) {
      throw new Error(`Uguu upload failed to return success data`);
    }

    const url = data.files[0].url;
    console.log(`[VideoGenerator] Video uploaded to Uguu: ${url}`);
    return url;
  } catch (err) {
