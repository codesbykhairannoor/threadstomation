...
async function uploadVideoToSupabase(buffer) {
  try {
    ...
    console.log(`[VideoGenerator] Bypassing Supabase, uploading video to Catbox.moe (PERMANENT): ${fileName}`);
    ...
    const url = await res.text(); // Catbox returns the URL directly in plain text
    console.log(`[VideoGenerator] Video uploaded permanently to Catbox: ${url}`);
    return url;
  } catch (err) {
