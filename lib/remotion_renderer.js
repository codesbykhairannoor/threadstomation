...
export async function renderTranvasMotionReel(slides, masterPrompt = '', accountName = '@tranvas', caption = '') {
  console.log(`[Remotion-Renderer] 🎬 Delegating renderTranvasMotionReel to dynamic Generative Metaphor Engine for ${accountName}...`);
  try {
    const { generateVisualMetaphorSpec } = await import('./gemini_instagram.js');
    const spec = await generateVisualMetaphorSpec(caption || masterPrompt, masterPrompt, accountName, 2);
    return await renderGenerativeMetaphorReel(spec, accountName);
  } catch (err) {
    console.error('[Remotion-Renderer] Error in renderTranvasMotionReel delegation, using fallback:', err.message);
    const fallbackSpec = {
      primitive: 'OpticalLens',
      headline: 'Scattered Energy = 0. Focus = Laser.',
      primitiveData: { multiplier: 37.8 },
    };
    return await renderGenerativeMetaphorReel(fallbackSpec, accountName);
  }
}
...
export async function renderGenerativeMetaphorReel(spec, accountName = '@tranvas') {
  ...
  try {
    const watermark = accountName.startsWith('@') ? accountName : `@${accountName}`;
    const headline = spec.headline || 'Ideas In. Pure Flow Out.';

    // Map primitive names to standalone compositions or fallback to GenerativeVisualMetaphor
    const standaloneMap = {
      OpticalLens: 'OpticalLensVideo',
      OpticalLensVideo: 'OpticalLensVideo',
      HourglassFlow: 'HourglassFlowVideo',
      HourglassFlowVideo: 'HourglassFlowVideo',
      IcebergMastery: 'IcebergMasteryVideo',
      IcebergMasteryVideo: 'IcebergMasteryVideo',
      NewtonsCradle: 'NewtonsCradleVideo',
      NewtonsCradleVideo: 'NewtonsCradleVideo',
      DominoCascade: 'DominoCascadeVideo',
      DominoCascadeVideo: 'DominoCascadeVideo',
    };

    const targetCompositionId = standaloneMap[spec.primitive] || 'GenerativeVisualMetaphor';
    const inputProps = {
    ...
    console.log(`[Remotion-Renderer] [1/3] Bundling Remotion (${targetCompositionId})...`);
    const entryPoint = path.join(__dirname, '../remotion/index.js');
    const bundleLocation = await bundle({ entryPoint });

    console.log(`[Remotion-Renderer] [2/3] Rendering 1080x1920 30FPS (195 frames / 6.5s) for ${targetCompositionId}...`);
    const composition = await selectComposition({
      serveUrl: bundleLocation,
    ...
    const videoBuffer = fs.readFileSync(finalVideoPath);
    const storagePath = `instagram/tranvas-${targetCompositionId.toLowerCase()}-${sessionId}.mp4`;
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const { data: pubData } = supabase.storage.from('media').getPublicUrl(storagePath);
        publicVideoUrl = pubData.publicUrl;
        break;
      } catch (uploadErr) {
        console.warn(`[Remotion-Renderer] ⚠️ Upload attempt ${attempt}/3 failed: ${uploadErr.message}`);
        if (attempt === 3) throw uploadErr;
        await new Promise((r) => setTimeout(r, 2500));
      }
    }

    console.log(`[Remotion-Renderer] ✅ Generative Metaphor Reel ready: ${publicVideoUrl}`);
