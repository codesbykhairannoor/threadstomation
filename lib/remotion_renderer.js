...
/**
 * Render Visual Thinking Metaphor Reels for Tranvas
 * @param {Array} slides - Gemini slides
 * @param {string} masterPrompt - Master prompt
 * @param {string} accountName - Account handle (@tranvas)
 * @param {string} caption - Caption
 * @returns {Promise<string>} Public Supabase MP4 URL
 */
export async function renderTranvasMotionReel(slides, masterPrompt = '', accountName = '@tranvas', caption = '') {
  const sessionId = Date.now();
  const rawVideoPath = path.join(SCRATCH_DIR, `tranvas_raw_${sessionId}.mp4`);
  const finalVideoPath = path.join(SCRATCH_DIR, `tranvas_final_${sessionId}.mp4`);
  const tempFiles = [rawVideoPath, finalVideoPath];

  console.log(`[Remotion-Renderer] 🎬 Generating Tranvas Visual Metaphor Video for ${accountName}...`);

  try {
    const flipTriplets = [
      { from: 'BUSY', mid: 'CALM', to: 'FLOW' },
      { from: 'CHAOS', mid: 'ORDER', to: 'FOCUS' },
      { from: 'NOISE', mid: 'CLEAR', to: 'DEPTH' },
      { from: 'IDEAS', mid: 'FRAME', to: 'BUILD' },
      { from: 'STUCK', mid: 'RESET', to: 'GROW' },
      { from: 'PANIC', mid: 'PAUSE', to: 'PEACE' },
    ];
    const randomTriplet = flipTriplets[Math.floor(Math.random() * flipTriplets.length)];

    // Rotate across 4 distinct visual metaphor compositions
    ...
    const compId = compositionOptions[Math.floor(Math.random() * compositionOptions.length)];

    let inputProps = { watermark: accountName.startsWith('@') ? accountName : `@${accountName}` };
    if (compId === 'TranvasFlipVideo') {
      inputProps = {
        fromWord: randomTriplet.from,
        midWord: randomTriplet.mid,
        toWord: randomTriplet.to,
        watermark: accountName.startsWith('@') ? accountName : `@${accountName}`,
      };
    }

    console.log(`[Remotion-Renderer] [1/3] Bundling composition: ${compId}...`);
    const entryPoint = path.join(__dirname, '../remotion/index.js');
    const bundleLocation = await bundle({ entryPoint });

    console.log(`[Remotion-Renderer] [2/3] Rendering 1080x1920 30FPS frames for ${compId}...`);
    const composition = await selectComposition({
      serveUrl: bundleLocation,
      id: compId,
      inputProps,
    ...
    await renderMedia({
      ...
      onProgress: ({ progress }) => {
        const pct = Math.round(progress * 100);
        if (pct % 33 === 0) console.log(`   Progress: ${pct}%`);
      },
    });

    console.log(`[Remotion-Renderer] [3/3] Muxing ambient soundtrack and uploading to Supabase...`);
    const sampleAudio = path.join(SCRATCH_DIR, 'cth_analysis/audio_2.wav');

    await new Promise((resolve, reject) => {
      let cmd = ffmpeg().input(rawVideoPath);
      if (fs.existsSync(sampleAudio)) {
        cmd = cmd
          ...
          .outputOptions([
            ...
            '-c:a aac',
            '-b:a 192k',
            '-filter_complex', '[1:a]afade=t=out:st=9.8:d=1.2[a]',
            '-map', '0:v',
            '-map', '[a]',
            '-t', '11.0'
          ]);
      ...
      cmd.save(finalVideoPath).on('end', resolve).on('error', reject);
    });

    const videoBuffer = fs.readFileSync(finalVideoPath);
    const storagePath = `instagram/tranvas-reel-${sessionId}.mp4`;
    ...
    const { data: pubData } = supabase.storage.from('media').getPublicUrl(storagePath);
    const publicVideoUrl = pubData.publicUrl;
    console.log(`[Remotion-Renderer] ✅ Tranvas Reel ready: ${publicVideoUrl}`);
    ...
    console.error('[Remotion-Renderer] ❌ Render error for Tranvas:', err);
    for (const f of tempFiles) {
      try { if (fs.existsSync(f)) fs.unlinkSync(f); } catch (_) {}
    }
    throw err;
...
/**
 * Render a fully generative 11s Visual Metaphor Reel based on dynamic AI Motion Spec.
 * Renders any of the 8 mental models dynamically via Remotion GenerativeVisualMetaphor.
 * @param {Object} spec - Generated spec from Gemini
 * @param {string} accountName - Account handle (@tranvas)
 * @returns {Promise<string>} Public Supabase MP4 URL
 */
export async function renderGenerativeMetaphorReel(spec, accountName = '@tranvas') {
  const sessionId = Date.now();
  const rawVideoPath = path.join(SCRATCH_DIR, `gen_metaphor_raw_${sessionId}.mp4`);
  const finalVideoPath = path.join(SCRATCH_DIR, `gen_metaphor_final_${sessionId}.mp4`);
  const tempFiles = [rawVideoPath, finalVideoPath];

  console.log(`[Remotion-Renderer] 🎬 Compiling Generative Visual Metaphor: ${spec.primitive} for ${accountName}...`);

  try {
    const inputProps = {
      primitive: spec.primitive || 'ChaosToPrism',
      categoryBadge: spec.categoryBadge || 'Life Architecture',
      headlinePart1: spec.headlinePart1 || 'Your Brain Is For Ideas,',
      headlinePart2: spec.headlinePart2 || 'Not Holding Them.',
      narrativeSubtitle: spec.narrativeSubtitle || 'Scattered cognitive load drains 80% of daily momentum.',
      takeaway: spec.takeaway || 'Offload mental friction to a single unified second brain.',
      watermark: accountName.startsWith('@') ? accountName : `@${accountName}`,
      primitiveData: spec.primitiveData || {},
    };

    console.log(`[Remotion-Renderer] [1/3] Bundling GenerativeVisualMetaphor...`);
    const entryPoint = path.join(__dirname, '../remotion/index.js');
    const bundleLocation = await bundle({ entryPoint });

    console.log(`[Remotion-Renderer] [2/3] Rendering 1080x1920 30FPS (330 frames) for ${spec.primitive}...`);
    const composition = await selectComposition({
      serveUrl: bundleLocation,
    ...
    console.log(`[Remotion-Renderer] [3/3] Muxing ambient audio soundtrack & uploading to Supabase...`);
    const sampleAudio = path.join(SCRATCH_DIR, 'cth_analysis/audio_2.wav');
    ...
    const videoBuffer = fs.readFileSync(finalVideoPath);
    const storagePath = `instagram/tranvas-metaphor-${sessionId}.mp4`;
    ...
    const { data: pubData } = supabase.storage.from('media').getPublicUrl(storagePath);
    const publicVideoUrl = pubData.publicUrl;
    console.log(`[Remotion-Renderer] ✅ Generative Visual Metaphor Reel ready: ${publicVideoUrl}`);
    ...
    console.error('[Remotion-Renderer] ❌ Render error for Generative Visual Metaphor:', err);
    for (const f of tempFiles) {
      try { if (fs.existsSync(f)) fs.unlinkSync(f); } catch (_) {}
    }
    throw err;
