...
export async function renderTranvasMotionReel(slides, masterPrompt = '', accountName = '@tranvas', caption = '') {
  ...
  try {
    ...
    const randomPair = flipPairs[Math.floor(Math.random() * flipPairs.length)];

    // 50% chance between 3D Flip Tiles and Chaos-to-Structure Second Brain
    const isFlip = Math.random() > 0.5;
    const compId = isFlip ? 'TranvasFlipVideo' : 'TranvasChaosToStructureVideo';
    const inputProps = isFlip
      ? { fromWord: randomPair.from, toWord: randomPair.to, watermark: '@tranvas' }
      : { watermark: '@tranvas' };
    ...
    await new Promise((resolve, reject) => {
      ...
      if (fs.existsSync(sampleAudio)) {
        cmd = cmd.input(sampleAudio).outputOptions(['-c:v copy', '-c:a aac', '-b:a 192k', '-t', '6.33']);
      } else {
