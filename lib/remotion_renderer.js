...
export async function renderGenerativeMetaphorReel(spec, accountName = '@tranvas') {
  ...
  try {
    const inputProps = {
      ...
      headline: spec.headline || 'Ideas In. Pure Flow Out.',
      watermark: accountName.startsWith('@') ? accountName : `@${accountName}`,
    ...
    console.log(`[Remotion-Renderer] [2/3] Rendering 1080x1920 30FPS (195 frames / 6.5s) for ${spec.primitive}...`);
    const composition = await selectComposition({
      serveUrl: bundleLocation,
    ...
    console.log(`[Remotion-Renderer] [3/3] Muxing 6.5s ambient audio soundtrack & uploading to Supabase...`);
    const sampleAudio = path.join(SCRATCH_DIR, 'cth_analysis/audio_2.wav');
      ...
      if (fs.existsSync(sampleAudio)) {
        cmd = cmd
          ...
          .outputOptions([
            ...
            '-c:a aac',
            '-b:a 192k',
            '-filter_complex', '[1:a]afade=t=out:st=5.8:d=0.7[a]',
            '-map', '0:v',
            '-map', '[a]',
            '-t', '6.5',
          ]);
