...
/**
 * Deletes an array of media files from Supabase given their public URLs.
 * Ensures zero-storage retention after Meta finishes importing the media.
 * @param {string[]} urls
 */
export async function deleteMediaUrls(urls) {
  if (!supabase || !urls || !Array.isArray(urls) || urls.length === 0) return;

  try {
    const filePaths = [];
    for (const url of urls) {
      if (typeof url !== 'string') continue;
      // Match public URL pattern: .../storage/v1/object/public/media/(filePath)
    ...
    if (error) throw error;
    console.log(`[Supabase Storage] 🗑️ Cleaned up ${filePaths.length} file(s) from Supabase:`, filePaths);
  } catch (err) {
    console.warn(`[Supabase Storage] ⚠️ Failed to delete media files:`, err.message);
  }
}
...
export async function cleanupOldStorage() {
  ...
  try {
    // Ultra-aggressive cleanup: 30 minutes retention to keep Supabase storage at virtually 0 MB!
    // Meta finishes ingesting within 60-90 seconds, so anything older than 30 minutes is 100% dead weight.
    ...
    for (const prefix of prefixes) {
      try {
        ...
        const filesToDelete = data
          .filter(file => file.name !== '.emptyFolderPlaceholder' && file.id && new Date(file.created_at) < cutoffTime)
    ...
    if (totalDeleted > 0) {
      console.log(`[Supabase Cleanup] Aggressively deleted ${totalDeleted} expired media files from Supabase. (Storage = 0 MB)`);
    } else {
