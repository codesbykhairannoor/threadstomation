...
function buildSlideSvg(slideIndex, totalSlides, title, body, themeIndex) {
  ...
  const titleSvg = titleLines.map((line, i) => 
    `<text x="540" y="${TITLE_Y + i * 85}" font-family="Inter" font-size="70" fill="white" font-weight="900" text-anchor="middle" letter-spacing="1">${escapeXml(line)}</text>`
  ).join('');

  const BODY_Y = TITLE_Y + titleLines.length * 85 + 70;

  const bodySvg = bodyLines.map((line, i) => 
    `<text x="540" y="${BODY_Y + i * 60}" font-family="Inter" font-size="46" fill="white" font-weight="500" text-anchor="middle" opacity="0.95">${escapeXml(line)}</text>`
  ).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="60" y="100" font-family="Inter" font-size="32" fill="rgba(255,255,255,0.6)" font-weight="600">
    ${slideIndex + 1}/${totalSlides}
  </text>

  <!-- Brand mark top-right -->
  <text x="1020" y="100" font-family="Inter" font-size="40" fill="${palette.accent}" font-weight="700" text-anchor="end" opacity="0.9">
    ✦
  </text>
...
/**
 * Convert SVG buffer → JPEG buffer using resvg-js (for fonts) and sharp (for jpeg format)
 */
async function svgToJpeg(svgString) {
  const fontBold = fs.readFileSync(path.join(process.cwd(), 'lib', 'fonts', 'Inter-Bold.ttf'));
  const fontMedium = fs.readFileSync(path.join(process.cwd(), 'lib', 'fonts', 'Inter-Medium.ttf'));

  const resvg = new Resvg(svgString, {
    font: {
      fontBuffers: [fontBold, fontMedium],
  ...
  // TikTok API strictly prefers/requires JPEG for Photo Carousels!
  return sharp(pngBuf).jpeg({ quality: 90 }).toBuffer();
}

async function uploadToImgbb(jpegBuffer, fileName) {
  // Use catbox.moe as a free reliable fallback with no API key required
...
async function uploadToSupabase(jpegBuffer, fileName) {
  if (!supabase) {
    console.warn('[TikTok-Carousel] Supabase missing, falling back to ImgBB...');
    return await uploadToImgbb(jpegBuffer, fileName);
  }
  ...
  // TikTok API requires the domain of the image URL to be verified in the Developer Portal.
  // Since we verified threadstomation.vercel.app, we must proxy the Supabase image through Vercel.
...
export async function generateSlideImages(slides) {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    // Add a small delay between uploads to avoid Catbox rate limit
