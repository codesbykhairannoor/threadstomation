...
function buildSlideSvg(slideIndex, totalSlides, title, body, themeIndex) {
  ...
  const titleSvg = titleLines.map((line, i) => 
    `<text x="540" y="${TITLE_Y + i * 85}" font-family="Montserrat" font-size="70" fill="white" font-weight="900" text-anchor="middle" letter-spacing="1">${escapeXml(line)}</text>`
  ).join('');

  const BODY_Y = TITLE_Y + titleLines.length * 85 + 70;

  const bodySvg = bodyLines.map((line, i) => 
    `<text x="540" y="${BODY_Y + i * 60}" font-family="Montserrat" font-size="46" fill="white" font-weight="normal" text-anchor="middle" opacity="0.95">${escapeXml(line)}</text>`
  ).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="60" y="100" font-family="Montserrat" font-size="32" fill="rgba(255,255,255,0.6)" font-weight="600">
    ${slideIndex + 1}/${totalSlides}
  </text>

  <!-- Brand mark top-right -->
  <text x="1020" y="100" font-family="Montserrat" font-size="40" fill="${palette.accent}" font-weight="700" text-anchor="end" opacity="0.9">
    ✦
  </text>
...
/**
 * Render SVG to PNG buffer via resvg-js (Ensures fonts work on Vercel)
 */
 ...
 */
async function uploadToSupabase(imageBuffer, fileName, contentType = 'image/png') {
