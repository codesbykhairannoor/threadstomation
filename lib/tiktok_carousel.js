...
/**
 * Convert SVG buffer → PNG buffer via resvg-js (Guarantees perfect font rendering on Vercel)
 */
  ...
  const resvg = new Resvg(svgString, {
    font: {
      fontFiles: [
        ...
        path.join(__dirname, 'fonts', 'Inter-Medium.ttf')
      ],
...
/**
 * Upload PNG buffer to Supabase Storage → return public URL
 */
async function uploadToSupabase(pngBuffer, fileName) {
 ...
 * @returns {Promise<string[]>} - array of public PNG URLs
 */
export async function generateSlideImages(slides) {
  ...
  const promises = slides.map(async (slide, i) => {
    ...
    const fileName = `tiktok/${Date.now()}-${Math.floor(Math.random()*1000)}-slide-${i + 1}.png`;
    const url = await uploadToSupabase(pngBuf, fileName);
